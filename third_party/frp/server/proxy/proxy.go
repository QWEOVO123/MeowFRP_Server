...
func (pxy *BaseProxy) handleUserTCPConnection(userConn net.Conn) {
	...
	if observer := dpihook.CurrentConnectionObserver(); observer != nil {
		...
		if observer.IsInboundBlocked(pxy.Context(), info) {
			xl.Warnf("the user conn [%s] was blocked by inbound ip blacklist", content.RemoteAddr)
			return
		}
		connectionID = observer.RegisterTCPConnection(pxy.Context(), info, userConn, workConn)
		defer observer.UnregisterTCPConnection(connectionID)
	}
	metrics.Server.OpenConnection(name, proxyType)
	inspectedUserConn := pxy.wrapDPIUserConn(userConn, proxyType)
	inCount, outCount, _ := pxy.joinUserConnection(local, inspectedUserConn, proxyType, xl)
